    ...
    try:
        ...
        if len(final_suggestions) < per_page:
            try:
                ...
                params = { 'api_key': TMDB_API_KEY, 'language': 'pt-BR', 'sort_by': 'popularity.desc', 'with_watch_providers': WATCH_PROVIDERS_STRING, 'watch_region': 'BR', 'page': page }
        ...
        return jsonify({
            ...
            'has_more': len(final_suggestions) == per_page,
            'current_page': page
        })
...
def get_recommendations_based_on_watched(watched_title_id):
    ...
    try:
        ...
        filtered_recommendations = [item for item in recommendations if item.get('id') not in exclude_set]
...
def process_disliked_titles():
    ...
    try:
        ...
        for title_id in disliked_ids:
            try:
                details_endpoint = f"{TMDB_API_URL}/tv/{title_id}"
                params = { 'api_key': TMDB_API_KEY, 'language': 'pt-BR', 'append_to_response': 'keywords,genres' }
                response = requests.get(details_endpoint, params=params)
                if response.ok:
